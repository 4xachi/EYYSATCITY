...
export default function WhatIfReplayScreen({ suggestion, onClose, onRetry }: WhatIfReplayScreenProps) {
  if (!suggestion) {
    return (
      <div className="min-h-[100dvh] flex items-center justify-center p-4">
        <div className="w-full max-w-2xl bg-white rounded-3xl p-8 text-center space-y-6 shadow-xl border border-brand-navy/10">
          ...
          <div className="flex justify-center gap-4 pt-4">
            <button onClick={onClose} className="px-6 py-3 rounded-xl border border-brand-navy/20 font-bold hover:bg-brand-paper transition-colors text-sm">Back to Final Profile</button>
            <button onClick={onRetry} className="px-6 py-3 rounded-xl bg-brand-ink text-white font-bold hover:bg-brand-navy transition-colors text-sm">Try Again</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-[100dvh] flex items-center justify-center p-4 py-12">
      <div className="w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden border border-brand-navy/10">
        
        {/* Header */}
        <div className="bg-brand-navy p-6 sm:p-8 text-white relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-2">
            ...
            <p className="text-white/70 font-sans text-sm max-w-2xl">
        ...
        <div className="p-6 sm:p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4 p-5 sm:p-6 rounded-2xl bg-brand-green/5 border border-brand-green/20 relative">
              <div className="absolute -left-4 top-1/2 -translate-y-1/2 hidden md:flex items-center justify-center w-8 h-8 rounded-full bg-white shadow-sm border border-brand-navy/10 z-10 text-brand-navy/40 font-mono text-xs">
                VS
              </div>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4 pt-4 border-t border-brand-navy/10">
            <button onClick={onClose} className="px-6 py-4 rounded-xl border border-brand-navy/20 font-bold hover:bg-brand-paper transition-colors text-sm sm:w-1/2 text-center">Back to Final Profile</button>
            <button onClick={onRetry} className="px-6 py-4 rounded-xl bg-brand-ink text-white font-bold hover:bg-brand-navy transition-colors text-sm sm:w-1/2 text-center flex items-center justify-center gap-2">
